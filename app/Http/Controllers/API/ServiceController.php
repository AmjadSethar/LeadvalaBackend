...
class ServiceController extends Controller
{
    ...
    {
        $filterService = [];
        $services = Service::where(['status' => true])->with(['addresses', 'user'])->latest('created_at');
        ...
        if ($request->serviceId) {
            $services = $this->model->where('id', $request->serviceId)->with(['user', 'related_services']);
        }
        ...
        $services = $services->with(['categories' => function ($query) {
            $query->without('zones'); 
        }])->get();
        
        // Return the filtered services without the zones
