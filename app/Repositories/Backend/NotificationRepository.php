...
class NotificationRepository extends BaseRepository
{
    ...
    public function create($request)
    {
        ...
        return view('backend.push-notification.create', compact('services'));
    }
    ...
    public function sendNotification($request)
    {
        ...
        if ($request->notification_send_to === 'user') {
            $users = User::role(RoleEnum::CONSUMER)->whereNotNull('fcm_token')->pluck('fcm_token')?->all();
            ...
            $users = $this->user->role(RoleEnum::PROVIDER)->whereNotNull('fcm_token')->pluck('fcm_token')->all();
            foreach ($users as $token) {
                $notification = [
                    'message' => [
                        'data' => [
