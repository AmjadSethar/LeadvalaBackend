...
class ServicemanWalletRepository extends BaseRepository
{
    ...
    {
        $servicemen = $this->user->role(RoleEnum::SERVICEMAN)->latest()->pluck('name', 'id');
        ...
        try {
            ...
            if ($request->serviceman_id) {
                if ($request->type == 'credit') {
                    $wallet = $this->creditWallet($request->serviceman_id, $request->balance, WalletPointsDetail::ADMIN_CREDIT);
                    if ($wallet) {
                    ...
                    return $wallet = $this->debitWallet($request->serviceman_id, $request->balance, WalletPointsDetail::ADMIN_DEBIT, $request);
                }
            } else {
