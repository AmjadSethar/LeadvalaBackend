...
use App\Models\Service;
    ...
    class BookingRepository extends BaseRepository
    {
        ...
        public function updateBookingStatusLog($booking)
        {
        }
                    ...
                    switch ($paymentTransaction?->type) {
                        case 'wallet':
                            ...
                            if($currentRoleName === RoleEnum::SERVICEMAN){
                                $item = $this->servicemanWallet->findOrFail($request->item_id);
                            } else if($currentRoleName === RoleEnum::PROVIDER){
                                $item = $this->providerWallet->findOrFail($request->item_id);
                            } else {
                                $item = $this->wallet->findOrFail($request->item_id);
                            }
                            break;
                        case 'subscription':
            ...
            if ($status) {
                if ($request->type == 'extra_charge' || $request->type == 'booking') {
                    if ($item->payment_method == 'wallet') {
                        ...
                        $this->debitWallet($item->consumer_id,  $item->total, WalletPointsDetail::WALLET_ORDER);
                    }

                    if ($item->parent_id) {
                        $item = $this->model->findOrFail($item->parent_id);
                    }

                    if ($item->parent_id) {
                        $parent = $this->model->findOrFail($item->parent_id)->first();
                        $parent?->update([
                            'payment_status' => $status,
                        ]);
                    ...
                    if ($request->is_type == 'extra_charge') {
