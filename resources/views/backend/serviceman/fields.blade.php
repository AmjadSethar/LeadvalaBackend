    ...
    <div class="tab-pane fade active show" id="general" role="tabpanel" aria-labelledby="general-tab">
                ...
                <div class="input-group mb-3 phone-detail">
                    <div class="col-sm-1">
                        <select class="select-2 form-control select-country-code" id="select-country-code"
                            ...
                            @php
                                $default = old('code', $serviceman->code ?? 1);
                            @endphp
                    ...
                    <select class="select-2 form-control" id="provider_id" name="provider_id"
                        ...
                        @foreach ($providers as $key => $option)
                            <option class="option" value="{{ $key }}"
                                @if (old('provider_id', isset($serviceman) ? $serviceman->provider->id : '') == $key) selected @endif>{{ $option }}</option>
                        @endforeach
                    </select>
                    @error('provider_id')
                        <span class="invalid-feedback d-block" role="alert">
        ...
        <div class="form-group row">
            <label class="col-md-2" for="experience_duration">{{ __('static.provider.experience_duration') }}<span>
                    *</span></label>
            <div class="col-md-10">
                <input class="form-control" id="experience_duration" type="number" min="1" name="experience_duration"
    ...
    <div class="tab-pane fade" id="address" role="tabpanel" aria-labelledby="address-tab">
                ...
                <div class="input-group mb-3 phone-detail">
                    <div class="col-sm-1">
                        <select class="select-2 form-control select-country-code" name="alternative_code"
                            data-placeholder="">
                            @php
                                $default = old('alternative_code', $serviceman->primaryAddress->code ?? 1);
                            @endphp
                            <option value="" selected></option>
                            @foreach (App\Helpers\Helpers::getCountryCodes() as $key => $option)
                                <option class="option" value="{{ $option->phone_code }}"
        ...
        @php
            if (isset($serviceman->primaryAddress->country_id) || old('country_id')) {
                $states = \App\Models\State::where('country_id', old('country_id', @$serviceman->primaryAddress->country_id))->get();
            } else {
                $states = [];
            }
        @endphp
        <div class="form-group row">
            ...
            <div class="col-md-10 error-div select-dropdown">
                ...
                @endphp
                <select class="select-2 form-control select-state" data-placeholder="{{ __('static.users.select_state') }}" id="state_id" name="state_id">
                    <option value=""></option>
                    @if(count($states)) 
        ...
        <div class="form-group row">
            ...
            <div class="col-md-10">
                <input class="form-control" type="text" name="area"
                    value="{{ $serviceman->primaryAddress->area ?? old('area') }}"
        ...
        <div class = "form-group row">
            ...
            <div class="col-md-10">
                <textarea class = "form-control" id="address" placeholder="{{ __('static.users.enter_address') }}"
