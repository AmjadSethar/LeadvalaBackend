...
<div class="tab-content" id="servicemanTabContent">
    <div class="tab-pane fade active show" id="general" role="tabpanel" aria-labelledby="general-tab">
        <div class="form-group row">
            ...
            <div class="col-md-10">
                <input class="form-control" type="file" accept=".jpg, .png, .jpeg" id="image" name="image" accept=".jpg, .png, .jpeg">
                @error('image')
                ...
                <div class="input-group mb-3 phone-detail">
                    <div class="col-sm-1">
                        <select class="select-2 form-control select-country-code" id="select-country-code"
                            ...
                            @php
                                $default = old('code', $customer->code ?? 1);
                            @endphp
    ...
    <div class="tab-pane fade" id="address" role="tabpanel" aria-labelledby="address-tab">
        @php
            if (isset($customer->primaryAddress->country_id) || old('country_id')) {
                $states = \App\Models\State::where('country_id',old('country_id', @$customer->primaryAddress->country_id))->get();
            } else {
                $states = [];
            }
        @endphp
                ...
                <div class="input-group mb-3 phone-detail">
                    <div class="col-sm-1">
                        <select class="select-2 form-control select-country-code" name="alternative_code"
                            data-placeholder="">
                            @php
                                $default = old('alternative_code', $customer->primaryAddress->code ?? 1);
                            @endphp
                            <option value="" selected></option>
                            @foreach (App\Helpers\Helpers::getCountryCodes() as $key => $option)
                                <option class="option" value="{{ $option->phone_code }}"
                ...
                <select class="select-2 form-control select-country" id="country_id" name="country_id"
                    ...
                    @forelse ($countries as $key => $option)
                        <option class="option" value={{ $key }}
                            @if (old('country_id', isset($customer->primaryAddress->country_id) ? $customer->primaryAddress->country_id : '') == $key) selected @endif>{{ $option }}</option>
                    @empty
        ...
        <div class = "form-group row">
            ...
            <div class="col-md-10">
                <textarea class = "form-control" id="address" placeholder="{{ __('static.users.enter_address') }}" rows="4"
